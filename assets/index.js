(()=>{
  ...
  const applyFilters=()=>{
    ...
    let n=0;
    tiles().forEach(t=>{
      const ok=(players==='all'||t.dataset.squad===players)&&(time==='all'||t.dataset.time===time)&&(!q||t.dataset.search.includes(q));
  ...
  const clearFinder=()=>{$$('#finder-players,#finder-time').forEach(s=>s.value='all');const q=$('.search input');if(q)q.value='';applyFilters()};
  const updateSavedUI=()=>{
